...
object TestMain extends App
{
 	val p = Prolog.parseProgram( """
		...
		value(C1), value(C2), value(C3), value(C4), value(C5), value(C6), value(C7), value(C8), value(C9), 
		...
		value(F1), value(F2), value(F3), value(F4), value(F5), value(F6), value(F7), value(F8), value(F9), 
		...
		value(I1), value(I2), value(I3), value(I4), value(I5), value(I6), value(I7), value(I8), value(I9), 
...
//  	Prolog.listing( db.code(Indicator('right, 3)).get )
	...
	val q = Prolog.parseQuery( """ Puzzle = [
			[3,_,_, _,8,_, 9,_,_],
			[_,2,6, 5,_,_, _,7,_],
			[4,_,_, _,_,7, _,5,2],
			[_,4,2, _,5,8, 1,3,_],
